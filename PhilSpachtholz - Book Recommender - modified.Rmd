...
content <- fread('../books.csv')
ratings <- fread('../ratings.csv')
item_tags <- fread('../book_tags.csv')
tags <- fread('../tags.csv')
```
...
For this step we select users that have in common that they rated the same books. To make it easier let's select one example user "David" EDIT changed from 17329 (user_id: 7329). First we select users that rated at least one book that David also rated. In total there are 440 users who have at least one book in common. 
...
user2 <- data.frame(item=colnames(ratingmat),rating=ratingmat["8",]) %>% filter(!is.na(rating))
tmp<-merge(user1, user2, by="item")
tmp
cor(tmp$rating.x, tmp$rating.y, use="pairwise.complete.obs")

user2 <- data.frame(item = colnames(ratingmat), rating = ratingmat["9", ]) %>% filter(!is.na(rating))
tmp <- merge(user1, user2, by="item")
tmp
cor(tmp$rating.x, tmp$rating.y, use="pairwise.complete.obs")
...
```
```{r}
summary(sim_mat[1:10])
glimpse(sim_mat)
random_users <- selected_users[c(1:4,60:90)]
qgraph(sim_mat[c(current_user, random_users), c(current_user, random_users)], layout = "spring", vsize = 4, theme = "TeamFortress", labels = c(current_user, random_users))
```
